...
const uploadImgs = (req, res, next) => {
  upload(req, res, async (err) => {
    ...
    if (!req.files || req.files.length === 0) {
      // Nếu không có file nào được chọn, tiếp tục chạy hàm middleware tiếp theo
      return next();
    }

    // Các bước xử lý khi có file được chọn
    try {
      const uploadPromises = req.files.map((file) => {
        // Tạo đường dẫn trong Firebase Storage
        const filePath = `room/${uuidv4()}${path.extname(file.originalname)}`;
